...
{
   # Obtain the full path to SubCountry module, defined in the %INC hash.

   my $input_xml_file = $INC{"Locale/SubCountry.pm"};
   # Now substitute the name of the data file
   $input_xml_file =~ s/SubCountry\.pm$/iso_3166_2.xml/;
   print($input_xml_file);

    my $xml_fh = new FileHandle "<$input_xml_file" or die "Could not locate input file : $input_xml_file $!";

    while ( my $current_line = $xml_fh->getline )
    {
        chomp($current_line);
